...
- **M1 status (2026-09-24):** built and passing mocked tests, not yet tested in-game. Spec auto-detection uses known tree-specific spells only; `Compat.GetTalentPointsByTree()` is a stub until a level 10+ probe shows how C_Traits stores Forever's three trees. Test files: `tests/m1_flows.lua` (flows) and `tests/logic_spec.lua` (pure logic).
- **Probe #1 findings that shape M2** (details in `docs/PROBE_LOG.md`): player aura queries return nothing in combat, so BuffSentinel and SealTracker must hold the last out-of-combat state and use whatever probe v2 finds readable (cast events, aura instance IDs, the shapeshift/aura stance). Cooldown `isActive`/`isEnabled` stay readable in combat, and duration objects drive the swipes. Player health, power and absorbs are secret even out of combat.
- **M2 status (2026-09-24):** BuffSentinel, SealTracker, CooldownHUD and Services/AuraService are built and pass mocked tests (`tests/m2_flows.lua`); not yet tested in-game. AuraService freezes the snapshot when combat reads come back empty or secret, predicts Seals from `UNIT_SPELLCAST_SUCCEEDED` (only if that spell ID is readable, which probe v2 will confirm), and reads the Paladin aura from the stance bar. CooldownHUD uses readable numbers out of combat and duration objects in combat. The group-blessing scan from 7.1 moves to M4, because it needs BlessingManager assignments.
- **Mocks:** unknown Capitalized keys on mock frames are no-op methods, and other keys read as nil, as on real frames.
- **Combat probe #2 (2026-09-24):** own `UNIT_SPELLCAST_SUCCEEDED` spell IDs are readable in combat. Player aura reads throw in combat, so check `C_Secrets.ShouldAurasBeSecret()` first. Cooldown `isActive`/`isOnGCD` are readable. Addon messages are restricted in combat (`Compat.CanSendAddonMessages`). Full details in `docs/PROBE_LOG.md`.
- **Owner preference:** Hammer of Justice belongs on the Holy bar (utility solo and in groups). Cleanse/Purify only matter in groups, so the Holy defaults use `CLEANSE@group` / `PURIFY@group` (a `@group` entry shows only in a party or raid; toggle with `/ptk cd group <spell>`). DB v2 migration upgrades Holy lists that still match the old default.
- **Dev helper:** `python tests/read_sv.py probe.X probeCombat.events depth=N` prints parts of the saved probe data.
- **M3 status (2026-09-24):** TankKit, the SealTracker Echo indicator and the AuraService cast predictions (`AS:PredictCast`: Seal, RF, Echo, Iron Creed) are built and pass `tests/m3_flows.lua`; not yet tested in-game (the owner is level 8, and RF and Prot/Ret talents come later). **Unverified guesses to replace once seen:** Echo lasts 10s and Iron Creed 6s (`DEFAULT_DURATION` in AuraService, overridden by durations learned out of combat), the Echo aura name ("Echo"), and which Seals are twistable (Command/Righteousness/Fury/Justice, per the handoff). Icon cooldown logic is now shared as `IconMixin:RefreshCooldown`. DB v3 migration upgrades untouched Prot/Ret lists.
- **Owner design direction (2026-09-24):** (1) **"Holy" theme**: classic WoW holy imagery, gold and parchment-light, Blizzard's gold dialog borders and holy spell icons. Every window and header goes through `UI/Theme.lua`. (2) **A missing Seal matters only in combat**: out of combat it's "idle" (hidden, no sound). (3) **Keep the PallyPower-style one-press buff button** (one key press = one cast; the addon only picks the target and spell, out of combat).
- **M4 status (2026-09-24):** Logic/Blessings.lua (suggest, codec, conflicts), Services/Comm.lua (queued, throttled, never sends in combat, during encounters or outside party/raid), Services/Roster.lua (out-of-combat group scan), Modules/BlessingManager.lua (secure button `NyteLytePaladinToolkitBuffButton`, CLICK keybind), UI/BlessingGrid.lua. Tests in `tests/m4_flows.lua` and `tests/logic_spec.lua`. Not yet in-game. Unknown: whether party buffs are readable out of combat (the probe's new `group` section checks, and Roster treats unreadable as "unknown", never "missing"). The anchor holding the secure button is marked `protected`, and Frames defers moving or resizing it until after combat.
- **M5 status (2026-09-24):** 0.5.0 is packaged with `python tools/package.py` (into `dist/`, gitignored). README is rewritten, with screenshots in `docs/screenshots/` taken before the M5 UI fixes (to be refreshed). The launch-day checklist is in `docs/PROBE_LOG.md`. **Not done yet:** a public GitHub repo, a CurseForge project (owner decision; the account is Nyte-Fyre), talent detection (needs the level 10 probe), a party-aura readability check, and a two-paladin sync test. The test runner also fails on leaked globals (only `NyteLytePaladinToolkit*`, `SLASH_NYTELYTEPALADINTOOLKIT*` and `BINDING_*` are allowed).
- **Level 10 probe (2026-09-24):** talent detection is implemented. Forever uses one C_Traits tree with three columns (`SpecDecision.TREE_X_BOUNDS`: x < 4000 Holy, < 8000 Prot, else Ret). Seal of Fury isn't a talent (it's a trained spell), Holy Shield exists, and there are no Greater Blessings. Details in `docs/PROBE_LOG.md`.
- **Party probe (2026-09-24):** party buffs are readable out of combat, and addon messages to PARTY are delivered. `C_ChatInfo.AreOutgoingAddonChatMessagesRestricted()` returns true even when sending works, so never gate on it; use `C_RestrictedActions.IsAddOnRestrictionActive(Enum.AddOnRestrictionType.*)` (see `Compat.CanSendAddonMessages`). Saved variables loaded normally again; the owner says Blizzard knows about the saving bug.
